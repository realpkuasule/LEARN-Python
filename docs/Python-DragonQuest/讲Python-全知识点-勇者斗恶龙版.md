...
| 2        | 第一个 Python 程序      | 4      | 在网站训练场运行第一行代码         |
...
> **核心理念**：在冒险者公会登记，直接使用网站训练场写出勇者的第一句话；本地环境是可选扩展。

### 2.1 在网站训练场运行代码

本课所有必做内容都在学习网站完成，无需安装 Python。在右侧「Python 代码」编辑器输入代码，点击「运行并挑战」，由网站沙箱执行并在下方显示结果。

**类比**：公会已经备好训练武器。勇者可以先学会挥剑，不必先自己打造铁匠铺。

### 2.2 运行你的第一行 Python

在右侧编辑器输入 `print("勇者降临！")`，点击运行——结果区出现「勇者降临！」
...
### 2.4 可选扩展：安装本地 Python

想在课程网站之外继续练习时，可以从 python.org 安装受支持的 Python 3，并体验终端、`>>>` 交互模式和 `.py` 文件。该扩展不参与课程通关、奖励或解锁。
...
```python
hp = 100        # 当前作用域中的名字 hp 绑定到整数对象
mp = 50         # 名字 mp 绑定到右侧求值得到的对象
name = "勇者刘老三"  # 名字 name 绑定到字符串对象

# 挨了怪物一下
hp = hp - 15    # 先计算右侧，再把 hp 重新绑定到结果对象
```

**类比**：公会名册把名字登记为指向某件物品。`b = a` 不复制物品，而是让两条记录指向同一个对象；更新 `a` 的记录不会自动更新 `b`。

**模型边界**：盒子比喻能粗略解释简单赋值，却无法可靠解释对象共享。名字存在于当前作用域的命名空间中，标签不是真的保存在对象身上；旧对象是否仍在内存中是实现细节。
...
**冒险思维**：HP 通常设计成整数，暴击率和属性倍率通常使用浮点数。本章先学习常见的 `int` 和 `float`；Python 还有其他数字类型，需要时再认识。
...
**类比**：勇者状态栏用布尔值表示真假状态。`None` 表示值的缺失；任务系统可以把这种缺失解释为「尚未获得任务物品」，但这只是当前游戏的业务约定。
...
**关键陷阱**：`=` 用于赋值；当左侧是简单名字时，它会建立或更新名字与对象的绑定。`==` 用于比较两边的值是否相等。
...
**类比**：战斗日志——攻击动作是固定的模板（「**_攻击了_**！造成了\_\_\_点伤害！」），名字和数字是活的。f-string 就是战斗日志生成器。
...
# 在战斗中使用
# hero 现在有最大生命值了（升过级）
hero = {"name": "勇者刘老三", "hp": 85, "max_hp": 100, "mp": 40, "max_mp": 50}
...
hero_crit_rate: float = 0.15  # 暴击率使用浮点数
...
| 编程概念     | RPG 映射             | 一句话                                 |
| ------------ | -------------------- | -------------------------------------- |
| 变量         | 公会名册记录         | 当前作用域中的名字绑定到对象           |
| int          | HP/等级              | 血量只有整数，没有半滴                 |
| float        | 暴击率/属性倍率      | 15% 暴击，1.5 倍克制                   |
| str          | 角色名/称号          | 「勇者刘老三」，后期获得称号拼成完整名 |
| bool         | 存活/阵亡            | 活着 True，死了 False                  |
| None         | 空缺的任务物品栏     | 表示值的缺失；具体含义由程序约定       |
