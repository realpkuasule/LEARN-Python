...
name = "勇者刘老三"  # 给文字贴了标签「名字」

# 挨了怪物一下
hp = hp - 15    # 把标签 hp 从 100 撕下来，贴到 85 上
```

**类比**：你的冒险者铭牌。铭牌上写着「勇者刘老三」，不是你本人变成了一张纸——铭牌只是贴在身上的标签。受伤了，HP 标签从「100」换成「85」，人还是同一个人。
...
**关键陷阱**：`=` 是贴标签，`==` 是「一样吗」。「勇者刘老三」写在铭牌上（=）和「这是勇者刘老三吗」（==）是两件完全不同的事。搞混了这个，AI 的代码会出神秘 bug。
